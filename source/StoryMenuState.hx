...
class StoryMenuState extends MusicBeatState
{
	...
	var classroom:FlxSprite;
	var elevater:FlxSprite;
	var baldi:FlxSprite;
	...
	override function create()
	{
		...
		scoreText = new FlxText(775, 150, 0, "SCORE: 49324858", 36);
		scoreText.setFormat(FontList.comic, 32);

		txtWeekTitle = new FlxText(FlxG.width * 0.7, 10, 0, "", 32);

		txtWeekTitle.setFormat(FontList.comic, 32, FlxColor.WHITE, RIGHT);
		txtWeekTitle.visible = false;
		txtWeekTitle.alpha = 0.7;

		var rankText:FlxText = new FlxText(0, 10);
		rankText.text = 'RANK: GREAT';
		rankText.setFormat(FontList.comic, 32);
		...
		classroom = new FlxSprite().loadGraphic(Paths.image('baldi/baldi_storymenu', 'shared'));
		classroom.antialiasing = ClientPrefs.globalAntialiasing;
		classroom.setGraphicSize(FlxG.width, FlxG.height);
		classroom.updateHitbox();
		add(classroom);

		var baldi:FlxSprite = new FlxSprite(400, 50);
		baldi.frames = Paths.getSparrowAtlas('characters/baldihappy', 'shared');
		baldi.animation.addByPrefix('idle', 'idle', 24, true);
		...
		elevater = new FlxSprite().loadGraphic(Paths.image('baldi/storymenu', 'shared'));
		elevater.antialiasing = ClientPrefs.globalAntialiasing;
		elevater.setGraphicSize(FlxG.width, FlxG.height);
		elevater.updateHitbox();
		add(elevater);
		...
		for (i in 0...WeekData.weeksList.length)
		{
			...
			weekThing.setGraphicSize(Std.int(weekThing.width / 1.5));
		...
		leftArrow = new FlxSprite(640, 130);
		...
		{
			var sprDifficulty:FlxSprite = new FlxSprite(600).loadGraphic(Paths.image('menudifficulties/' + CoolUtil.difficultyStuff[i][0].toLowerCase()));
			// sprDifficulty.x += (308 - sprDifficulty.width) / 2;
			sprDifficulty.ID = i;
			sprDifficulty.antialiasing = ClientPrefs.globalAntialiasing;
			sprDifficulty.setGraphicSize(Std.int(sprDifficulty.width / 2.8));
		...
		rightArrow = new FlxSprite(1000, 1000);
	...
	function changeDifficulty(change:Int = 0):Void
	{
		...
		sprDifficultyGroup.forEach(function(spr:FlxSprite)
		{
			...
			if (curDifficulty == spr.ID)
			{
				...
				FlxTween.tween(spr, {y: leftArrow.y + 35, alpha: 1}, 0.07);
			}
			...
			switch (spr.ID)
			{
				case 0:
					spr.x = 600;
				// spr.setGraphicSize(Std.int(spr.width / 2.8));
				case 1:
					spr.x = 590;
