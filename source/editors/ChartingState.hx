...
class ChartingState extends MusicBeatState
{
	...
	var eventStuff:Array<Dynamic> = [
		...
		['Baldi Jumpscare', 'Nothing. Just Don\'t spam this event;;']
	];
	...
	**/
	...
	var strumLineGroup:FlxSpriteGroup;
	...
	override function create()
	{
		...
		strumLineGroup = new FlxSpriteGroup();
		add(strumLineGroup);
		...
		for (i in 0...8)
		{
			var strumLine:FlxSprite = new FlxSprite(-70, -100);

			strumLine.frames = Paths.getSparrowAtlas('NOTE_assets');

			switch (Math.abs(i))
			{
				case 0: // Left 1
					strumLine.x = GRID_SIZE * 0;
					strumLine.animation.addByPrefix('P0', 'arrowLEFT', 24, true);
					strumLine.animation.addByPrefix('0', 'left confirm', 24, false);
					strumLine.animation.play('P0');
				case 1: // Down 1
					strumLine.x = GRID_SIZE * 1;
					strumLine.animation.addByPrefix('P1', 'arrowDOWN', 24, true);
					strumLine.animation.addByPrefix('1', 'down confirm', 24, false);
					strumLine.animation.play('P1');
				case 2: // Up 1
					strumLine.x = GRID_SIZE * 2;
					strumLine.animation.addByPrefix('P2', 'arrowUP', 24, true);
					strumLine.animation.addByPrefix('2', 'up confirm', 24, false);
					strumLine.animation.play('P2');
				case 3:
					strumLine.x = GRID_SIZE * 3;
					strumLine.animation.addByPrefix('P3', 'arrowRIGHT', 24, true);
					strumLine.animation.addByPrefix('3', 'right confirm', 24, false);
					strumLine.animation.play('P3');
				case 4:
					strumLine.x = GRID_SIZE * 4;
					strumLine.animation.addByPrefix('P4', 'arrowLEFT', 24, true);
					strumLine.animation.addByPrefix('4', 'left confirm', 24, false);
					strumLine.animation.play('P4');
				case 5:
					strumLine.x = GRID_SIZE * 5;
					strumLine.animation.addByPrefix('P5', 'arrowDOWN', 24, true);
					strumLine.animation.addByPrefix('5', 'down confirm', 24, false);
					strumLine.animation.play('P5');
				case 6:
					strumLine.x = GRID_SIZE * 6;
					strumLine.animation.addByPrefix('P6', 'arrowUP', 24, true);
					strumLine.animation.addByPrefix('6', 'up confirm', 24, false);
					strumLine.animation.play('P6');
				case 7:
					strumLine.x = GRID_SIZE * 7;
					strumLine.animation.addByPrefix('P7', 'arrowRIGHT', 24, true);
					strumLine.animation.addByPrefix('7', 'right confirm', 24, false);
					strumLine.animation.play('P7');
			}
			// 40 x 40
			strumLine.setGraphicSize(GRID_SIZE, GRID_SIZE);
			// Set Offsets to

			strumLine.offset.set(18, 60);
			//
			add(strumLine);
			//
			strumLine.ID = i;
		...
		camPos.setPosition(strumLineGroup.members[0].x + CAM_OFFSET, strumLineGroup.members[0].y);
		...
		zoomTxt.setFormat(FontList.comic, 16);
	...
	function addSongUI():Void
	{
		...
		var stepperSpeed:FlxUINumericStepper = new FlxUINumericStepper(10, stepperBPM.y + 35, 0.1, 1, 0.1, 15, 1);
	...
	override function update(elapsed:Float)
	{
		...
		if (PlayState.SONG.song.toLowerCase() == 'lesson' && curStep == 1156 && curSection == 72)
		{
			updateHeads();
			FlxG.camera.shake(0.02, 0.1);
		}
		else if (PlayState.SONG.song.toLowerCase() == 'lesson' && curStep >= 1 && curStep < 1155 && curSection == 72)
		{
			updateHeads();
		}
		...
		/*
			...
			{
				for (i in 0...strumLineGroup.length)
				{
		...
		for (i in 0...strumLineGroup.length)
		{
			strumLineGroup.members[i].y = getYfromStrum((Conductor.songPosition - sectionStartTime()) / zoomList[curZoom] % (Conductor.stepCrochet * _song.notes[curSection].lengthInSteps));
		}
		camPos.y = strumLineGroup.members[0].y;

		if (!disableAutoScrolling.checked)
		{
			if (strumLineGroup.members[0].y > (gridBG.height / 2))
			{
				trace(curStep);
			...
			else if (strumLineGroup.members[0].y < -10)
			{
				changeSection(curSection - 1, false);
			}
		}
		...
		curRenderedNotes.forEachAlive(function(note:Note)
		{
				...
				/*
					if (note.strumTime > lastConductorPos && FlxG.sound.music.playing && note.noteData > -1)
					{
						for (i in 0...strumLineGroup.length)
						{
							...
							{
								strumLineGroup.members[i].animation.play('' + (note.noteData % 4 + 4));
								if (!note.mustPress)
								{
									strumLineGroup.members[i].animation.play('' + note.noteData % 4);
								}
							}
							else
							{
			...
			/*
				for (i in 0...strumLineGroup.length)
				{
					if (!_song.notes[curSection].mustHitSection)
					{
						if (strumLineGroup.members[i].animation.curAnim.name == '' + note.noteData % 4
							&& strumLineGroup.members[i].animation.curAnim.finished
							&& !note.mustPress)
						{
							strumLineGroup.members[i].animation.play('P' + note.noteData % 4);
						}
						if (strumLineGroup.members[i].animation.curAnim.name == '' + note.noteData % 4
							&& strumLineGroup.members[i].animation.curAnim.finished
							&& note.mustPress)
						{
							strumLineGroup.members[i].animation.play('P' + (note.noteData % 4 + 4));
						}
					}
					if (_song.notes[curSection].mustHitSection)
					{
	...
	function updateHeads():Void
	{
		...
		if (PlayState.SONG.song.toLowerCase() == 'lesson' && curStep >= 1156 && curStep < 1631)
		{
			healthIconP2 = 'baldimad';
		}
		else if (PlayState.SONG.song.toLowerCase() == 'lesson' && curStep >= 1 && curStep < 1156)
		{
			healthIconP2 = 'baldi';
		}
		...
		if (_song.notes[curSection].mustHitSection)
		{
			...
			strumLineGroup.members[0].x = GRID_SIZE * 4;
			strumLineGroup.members[1].x = GRID_SIZE * 5;
			strumLineGroup.members[2].x = GRID_SIZE * 6;
			strumLineGroup.members[3].x = GRID_SIZE * 7;

			strumLineGroup.members[4].x = GRID_SIZE * 0;
			strumLineGroup.members[5].x = GRID_SIZE * 1;
			strumLineGroup.members[6].x = GRID_SIZE * 2;
			strumLineGroup.members[7].x = GRID_SIZE * 3;
		}
		else
		{
			leftIcon.changeIcon(healthIconP2);
			rightIcon.changeIcon(healthIconP1);
			strumLineGroup.members[0].x = GRID_SIZE * 0;
			strumLineGroup.members[1].x = GRID_SIZE * 1;
			strumLineGroup.members[2].x = GRID_SIZE * 2;
			strumLineGroup.members[3].x = GRID_SIZE * 3;

			strumLineGroup.members[4].x = GRID_SIZE * 4;
			strumLineGroup.members[5].x = GRID_SIZE * 5;
			strumLineGroup.members[6].x = GRID_SIZE * 6;
			strumLineGroup.members[7].x = GRID_SIZE * 7;
		}
	}
